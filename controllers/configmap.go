...
func (r *CephClusterReconciler) syncConfigMap() error {
	...
	klog.Infof("syncConfigMap: creating config map %s", AccessConfigMapName)
...
func (r *CephClusterReconciler) getConfigMap() (*corev1.ConfigMap, error) {
	cm := &corev1.ConfigMap{}
	if err := r.Client.Get(context.TODO(), types.NamespacedName{Namespace: r.Cluster.Namespace, Name: AccessConfigMapName}, cm); err != nil {
		return nil, err
	}
	return cm, nil
}
